...
export function ListaComprasView({ itens, insumos: initialInsumos = [], historico: initialHistorico = [], onSaveInsumos, onSaveHistorico, onUpdateEstoque }: Props) {
  ...
  const atualizarInsumo = (nome: string, patch: Partial<Insumo>) => {
    const atualizados = insumos.map((i) => i.nome === nome ? normalizar({ ...i, ...patch }) : i)
    setInsumos(atualizados)
    onSaveInsumos?.(atualizados)
  }
  const registrarCompra = () => {
    const item = insumos.find((i) => i.nome === compra.nome)
    ...
    atualizarInsumo(item.nome, { precoReferencia: unitario, precoCompra: unitario, ultimaAtualizacaoPreco: compra.data })
