...
export function CmvView({ insumos, fichas, userRole, onSaveInsumos, onSaveFichas }: { insumos: Insumo[]; fichas: FichaTecnica[]; userRole: string; onSaveInsumos: (data: Insumo[]) => void; onSaveFichas: (data: FichaTecnica[]) => boolean }) {
  ...
  const [confirmacao, setConfirmacao] = useState<"cancelar" | "desativar" | null>(null)
  const [draftOriginal, setDraftOriginal] = useState<FichaTecnica | null>(null)
  const [fichaPendenteStatus, setFichaPendenteStatus] = useState<FichaTecnica | null>(null)
  ...
  function abrirEdicao(ficha: FichaTecnica) {
    ...
    setDraftError("")
    setDraftOriginal(structuredClone(ficha))
    setDraftFicha(structuredClone(ficha))
  }

  function draftAlterado() {
    if (!draftFicha) return false
    if (!draftOriginal) return Boolean(draftFicha.nome.trim() || draftFicha.precoVenda || draftFicha.categoria || draftFicha.ingredientes.some((item) => item.insumoNome.trim() || item.quantidade > 0))
    return JSON.stringify(draftFicha) !== JSON.stringify(draftOriginal)
  }

  function fecharEditor() {
    if (draftAlterado()) return setConfirmacao("cancelar")
    setDraftFicha(null)
    setDraftOriginal(null)
  }

  function salvarFicha() {
    if (!canManageFichas) return setDraftError("Acesso negado: apenas owner ou admin podem salvar fichas.")
  ...
  function alterarStatus(ficha: FichaTecnica) {
    if (!canManageFichas) return
    if (ficha.ativo !== false) { setFichaPendenteStatus(ficha); return setConfirmacao("desativar") }
    onSaveFichas(fichas.map((item) => item.id === ficha.id ? { ...item, ativo: true } : item))
  }
  ...
  function salvarInsumo(data: Insumo) {
    if (!canManageInsumos) return
    onSaveInsumos(insumos.some((item) => item.id === data.id) ? insumos.map((item) => item.id === data.id ? data : item) : [...insumos, data])
  }
  ...
  return <div className="space-y-6">
    ...
    <AlertDialog open={confirmacao !== null} onOpenChange={(open) => !open && setConfirmacao(null)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{confirmacao === "desativar" ? "Desativar ficha técnica?" : "Descartar alterações?"}</AlertDialogTitle><AlertDialogDescription>{confirmacao === "desativar" ? "Esta ficha deixará de ser utilizada em novos lançamentos, mas continuará preservada no sistema e no histórico." : "Existem alterações nesta ficha que ainda não foram salvas."}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel onClick={() => setConfirmacao(null)}>{confirmacao === "desativar" ? "Cancelar" : "Continuar editando"}</AlertDialogCancel><AlertDialogAction onClick={() => { if (confirmacao === "desativar" && fichaPendenteStatus) { onSaveFichas(fichas.map((item) => item.id === fichaPendenteStatus.id ? { ...item, ativo: false } : item)); setFichaPendenteStatus(null) } else { setDraftFicha(null); setDraftOriginal(null) } setConfirmacao(null) }}>{confirmacao === "desativar" ? "Desativar ficha" : "Descartar alterações"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </div>
}
